...
namespace Tralala
{
	...
	{
		const std::array<BYTE, 11> heappattern = { 0x45, 0x33, 0xC0, 0x48, 0x8B, 0xC8, 0xBA, 0x80, 0x06, 0x00, 0x00 };
		g_scaleFormHeapAddr = (uintptr_t)scan_memory(heappattern, 0x44, false, 0x3, 0x7);
		...
		g_setDispInfoAddr = (uintptr_t)scan_memory(setpattern, 0xCD, false);

		const std::array<BYTE, 6> gmemberpattern = { 0x44, 0x8B, 0xE0, 0x49, 0x8B, 0xC6 };
		g_getMemberAddr = (uintptr_t)scan_memory(gmemberpattern, 0x1A, false, 0x1, 0x5);

		const std::array<BYTE, 8> smemberpattern = { 0xF3, 0x0F, 0xE6, 0xC0, 0x4C, 0x89, 0x75, 0x00 };
		g_setMemberAddr = (uintptr_t)scan_memory(smemberpattern, 0x2A, false, 0x1, 0x5);

		const std::array<BYTE, 8> textpattern = { 0xFF, 0x90, 0xE8, 0x02, 0x00, 0x00, 0xB0, 0x01 };
		g_setTextAddr = (uintptr_t)scan_memory(textpattern, 0x9A, false);
	}
