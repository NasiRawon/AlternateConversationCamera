...
#define APP_NAME "Alternate Conversation Camera"
...
template<std::size_t SIZE>
const BYTE* scan_memory(const std::array<BYTE, SIZE>& bytes_to_find, const BYTE offset,
	bool positive_offset, BYTE start = 0, BYTE size = 0)
{
	...
	return scan_memory_internal(base, minfo.SizeOfImage, bytes_to_find, offset, positive_offset, start, size);
}